...
local function AuthenticateCommon(Entity, EntityData, isDriver)
	...
	pcall(function()
		FriendlyName = EntityData.FriendlyName or "UNKNOWN APP TYPE"
		ValVS = PrismCore:ValidateVersionString(EntityData.Version)
	end)

	if not ValVS then 		
		wr("Install failed for ".. (tostring(FriendlyName or Entity.Name)) ..". Compatibility error. ")
		return nil
	end
	...
	if typeof(EntityData) == "table" and EntityData.Version and ValVS and EntityData.API and Entity then
		...
		if isDriver and (table.find(WhitelistedDrivers, Entity.Name)) then
			APIPackage.PrismCore = PrismCore
			APIPackage.Internal = PrismCore
		elseif isDriver then
			wr(EntityData.FriendlyName.." REQUESTED CORE DRIVER ACCESS, BUT IT IS NOT WHITELISTED. CORE ACCESS DENIED.")
			isDriver = false
		end
